...
const AllTestpapers = () => {
  ...
  const getAllNotes = async () => {
    ...
    const { data } = await axios.get(
      `http://localhost:5000/api/students/?page=${pageno}&limit=${perPageEntries}`
    );
    // map
    console.log("data", data);
    let newnotes = [...data.students];
                ...
                return (
                  <>
                    <Tr key={stud.sid}>
                      ...
                      <Td> {stud.email} </Td>
                      <Td> {stud.college} </Td>
                      <Td>{stud?.issupervisor ? "true" : "false"} </Td>

                      <Button p="3" mt="1" bg={"teal.200"}>
                        <Link to={`/note/${stud.nid}`}>View</Link>
                      </Button>
                    </Tr>
                  </>
                );
